...
启动时先读取 `localStorage`，随后自动请求 GitHub raw 文件。请求成功且数据有效时，用远程内容完整覆盖本地缓存，重新读取菜谱、菜单、历史和采购状态并立即渲染；请求失败时显示非阻断提示并继续使用已有本地缓存。空远程快照不会在自动同步时清除已有本地数据。
...
读取并校验成功后先要求用户确认，再覆盖本地缓存、重新加载应用状态并立即渲染，不要求浏览器刷新。
...
“数据与备份”页面保留本地 JSON 导入导出，并新增 GitHub 同步区：Token 密码输入框、“读取 GitHub”和“保存 GitHub”按钮、公开数据风险说明。读取、应用和保存期间按钮显示进行中状态；成功或失败同时通过 Toast 和页面内持久状态提示反馈。移动端底部导航包含“数据”，保证手机可完成同步。
